...
window.onload = function () {
    ...
    // appendMsg(initLeft)
    // var rightMsg = "fine"
    // var initRight = createRightDiv(userNickName, rightMsg);
    // appendMsg(initRight)
...
function wslink() {
    ...
    wsserver.onmessage = function (msg) {
        ...
        if(message.type == 'open'){
            var joinDiv = createJoinDiv(message.msg);
            userNickName = message.no
            appendMsg(joinDiv)
...
function sendMsg() {
    ...
    if (message !== '') {
        var newDiv = createRightDiv(userNickName, message);
        appendMsg(newDiv)
        text.value = '';
